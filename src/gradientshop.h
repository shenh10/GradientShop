...
private:
    ...
    double gradientOrientation( int x, int y);
    ...
    void _computeGradientMagnitude( Mat &dst, Mat &pdx, Mat &pdy);
    void _computeOrient();
    void _computeNormalizedMagnitude();
    void _computeEdgeLength();
    void _computeSaliency();
public:
    Mat src, orient, normaled_mag, len_edge, saliency_x, saliency_y, saliency_mag, u_x, u_y, u, u_mag;
    ...
    void writeCSV(Mat src, string filename);
};
